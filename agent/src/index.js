...
const langfuse_1 = __importDefault(require("langfuse"));
const path_1 = __importDefault(require("path"));
const heartbeat_js_1 = require("./heartbeat.js");
const downloads_js_1 = require("./collectors/downloads.js");
const gmail_js_1 = require("./collectors/gmail.js");
const db_js_1 = require("./db.js");
const dedup_js_1 = require("./pipeline/dedup.js");
const extractor_js_1 = require("./pipeline/extractor.js");
const relevance_js_1 = require("./pipeline/relevance.js");
const label_js_1 = require("./pipeline/label.js");
const drive_js_1 = require("./drive.js");
const metrics_js_1 = require("./metrics.js");
const langfuse = new langfuse_1.default({
...
async function getLastProcessedAt() {
    try {
        const rows = await (0, db_js_1.sql) `SELECT MAX(ingested_at) as last FROM "Item"`;
        return rows[0]?.last ? new Date(rows[0].last) : new Date(0);
    }
    catch {
        return new Date(0);
    }
...
async function handleFile(filePath) {
    ...
    const contentHash = await (0, dedup_js_1.computeHash)(filePath);
    if (await (0, dedup_js_1.isDuplicate)(contentHash)) {
        span.update({ output: { status: 'duplicate_skip' } });
        return;
    }
    // 2. Size-band content extraction
    const content = await (0, extractor_js_1.extractContent)(filePath);
    // 3. Stage 1: relevance gate
    const relevanceSpan = langfuse.trace({ name: 'relevance_gate', input: { filename, mimeType: content.mimeType } });
    const relevance = await (0, relevance_js_1.classifyRelevance)(filename, content.mimeType, content);
  ...
  `;
    ...
    const taxonomyRows = await (0, db_js_1.sql) `
  ...
  `;
    const existingTaxonomy = {
        types: taxonomyRows.filter((r) => r.axis === 'type').map((r) => r.name),
        froms: taxonomyRows.filter((r) => r.axis === 'from').map((r) => r.name),
        contexts: taxonomyRows.filter((r) => r.axis === 'context').map((r) => r.name),
    };
    // Stage 2: label classifier
    const labelSpan = langfuse.trace({ name: 'label_classifier', input: { filename } });
    const label = await (0, label_js_1.classifyLabel)(filename, content.mimeType, content.content, existingTaxonomy);
    ...
    const itemRows = await (0, db_js_1.sql) `
    UPDATE "Item"
  ...
  `;
    ...
    if (itemRows.length > 0) {
        try {
            const driveInboxId = await (0, drive_js_1.uploadToInbox)(filePath, content.mimeType);
            await (0, db_js_1.sql) `
        UPDATE "Item"
...
async function handleGmailMessage(msg) {
    // Gmail uses message ID as dedup key — hash the ID bytes
    const contentHash = await (0, dedup_js_1.computeHashFromBuffer)(Buffer.from(msg.id));
    if (await (0, dedup_js_1.isDuplicate)(contentHash))
        return;
    const gmailTraceClient = langfuse.trace({ name: 'ingest_gmail', input: { id: msg.id } });
    const gmailTraceId = gmailTraceClient.id;
    const relevance = await (0, relevance_js_1.classifyGmailRelevance)(msg);
...
(async () => {
    ...
    (0, heartbeat_js_1.startHeartbeat)(langfuse);
    const stopDownloads = (0, downloads_js_1.startDownloadsCollector)(langfuse, handleFile, lastProcessedAt);
    // Gmail polling: initial run on startup, then every 5 minutes
    const gmailPoll = async () => {
        try {
            await (0, gmail_js_1.pollGmail)(langfuse, handleGmailMessage);
        }
        catch (err) {
    ...
    const runSnapshot = async () => {
        try {
            await (0, metrics_js_1.snapshotMetrics)();
